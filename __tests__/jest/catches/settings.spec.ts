...
describe('check update settings routes catch', () => {
    test('throw in restriction id search', async () => {
        const response = await supertest(app).patch("/settings").send({restrictionName: "throw"}).set('Cookie', ['token=token_existing']);
        expect(response.statusCode).toBe(500)
    })
    test('throw in restriction connected to user search', async () => {
        const response = await supertest(app).patch("/settings").send({restrictionName: "throw2"}).set('Cookie', ['token=token_existing']);
        expect(response.statusCode).toBe(500)
    })
