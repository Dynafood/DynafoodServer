...
export const createShoppingList = async (req: Request, res: Response) => {
    ...
    if (!name || name.length === 0) {
        res.status(400).send({ Error: 'No name provided', Details: translations['name is not provided or empty!'] });
        return;
    }
    try {
...
export const deleteShoppingList = async (req: Request, res: Response) => {
    ...
    if (!id || id.length === 0 ||id == null) {
        res.status(400).send({ Error: 'No listId provided', Details: translations['listId is not provided or empty!'] });
        return;
    }
    try {
...
export const createShoppingListItem = async (req: Request, res: Response) => {
    ...
    if (!itemName || itemName.length === 0) {
        res.status(400).send({ Error: 'No itemName provided', Details: translations['itemName is not provided or empty!'] });
        return;
    }
...
export const deleteShoppingListItem = async (req: Request, res: Response) => {
    ...
    if (!id || id.length === 0) {
        res.status(400).send({ Error: 'No itemId provided', Details: translations['itemId is not provided or empty!'] });
        return;
    }
    try {
