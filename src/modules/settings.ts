...
export const getRestrictionIdByName = async (req: Request, res: Response, next: NextFunction) : Promise<void> => {
    try {
        ...
        if (restrictinoName == null) {
            res.status(400).send({ Error: '400 Bad Request', Details: 'Missing restrictionName' });
        }
        const restrictionID : string | null = await database.Settings.getRestrictionIdByName(restrictinoName);
        if (restrictionID == null) {
            res.status(404).json({ Error: '404 Not Found', Details: `The restriction ${req.body.restrictionName} is not available on dynafood!` });
            return;
        }
...
export const hasRestriction = async (req: Request, res: Response, next: NextFunction) : Promise<void> => {
    try {
        if (!await database.Settings.userHasRestriction(res.locals.user.userid, res.locals.restrictionID)) {
            res.status(400).send({ Error: '400 Bad Request', Details: `This user does not have a restriction for ${req.body.restrictionName}.` });
            return;
        }
        next();
    } catch (err: any) {
...
export const getSettings = async (req: Request, res: Response) : Promise<void> => {
    try {
        ...
        if (userSettings.length === 0) {
            res.status(204).send({ Error: '204 No Content', Details: 'no content' });
            return;
        }
