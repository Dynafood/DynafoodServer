...
describe('check get product routes', () => {
    ...
    test('test placeholder image', async () => {
        const response = await supertest(app).get("/products/barcode/40084060?language=fr").send().set('authorization', 'Bearer token_existing');
        expect(response.statusCode).toBe(200)
        expect(response.body.images).toEqual('http://x2024dynafood545437452001.westeurope.cloudapp.azure.com:8081/placeholderImage');
    })
    test('german', async () => {
        const response = await supertest(app).get("/products/barcode/402976400180?language=de").send().set('authorization', 'Bearer token_existing');
        expect(response.statusCode).toBe(204)
    })
    test('french', async () => {
        const response = await supertest(app).get("/products/barcode/402976400180?language=fr").send().set('authorization', 'Bearer token_existing');
        expect(response.statusCode).toBe(204)
    })

    test('vegan', async () => {
        const response = await supertest(app).get("/products/barcode/20949785?language=fr").send().set('authorization', 'Bearer token_existing');
        expect(response.statusCode).toBe(200)
    })
})
